...
2. Deploy test service and ping test pod
```
...
4. DNS test
```
kubectl exec -it test-alpine-777bf49f54-ngncl -- ping www.adobe.com
```
```
cisco@k8s-cp:~/cilium-egw/egw-ha$ kubectl exec -it test-alpine-777bf49f54-ngncl -- ping www.adobe.com
PING www.adobe.com (184.25.127.146): 56 data bytes
64 bytes from 184.25.127.146: seq=0 ttl=47 time=30.019 ms
64 bytes from 184.25.127.146: seq=1 ttl=47 time=18.867 ms
^C
```
...
2. update EGW blue with 0.0.0.0/0 destCIDR
```yaml
  destinationCIDRs:
  - "0.0.0.0/0"
  #- "10.0.0.0/8"
...
6. DNS test from pod inside the cluster
```
...
7.  curl test from pod in the cluster
```
~ $ curl http://10.96.163.83
<!DOCTYPE html>
<html>
